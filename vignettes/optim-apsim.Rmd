...
Once we have set up these pieces we can run the optimization. The main consideration is that it is almost always a good idea to set up lower and upper bounds on the parameters in terms how much higher and lower than the default values can they be. In a model such as APSIM usually plus or minus 50 percent is enough of a range and even a lower range can often suffice. Below these are included as 0.5 and 1.5. We also need to specify that these two parameters are in the 'replacements' part of the file because there are parameters which can be found in both the 'replacements' part and the main simulation section of an APSIM-X file.
