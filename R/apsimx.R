...
#' If there are multiple reports, it will return a list with names corresponding to the 
...
read_apsimx <- function(file = "", src.dir = ".", value = "report"){
  ...
  if(length(report.names) > 1L && value == "report"){
    ...
    for(i in seq_along(report.names)){
      lst0[[i]] <- DBI::dbGetQuery(con, paste("SELECT * FROM ", report.names[i]))  
    }
